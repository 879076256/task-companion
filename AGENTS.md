...
- Phase 0–9 已人工验收；Task Companion 1.0.0 已安装到经授权的正式 Vault。用户另行授权正式桌面/移动主页只在显示层隐藏 Task Companion ID；提交、标签、main 合并和对外发布仍需单独授权。
- 不实现多层树、依赖图、甘特图、AI 自动拆解、脚本市场、任意 JavaScript 执行、未经授权的正式主页修改、后台轮询、遥测、网络请求或外部服务。
- 未经用户明确确认，不扩大产品范围，不安装到 Vault，不发布，不提交 Git commit。
- 正式 Vault 安装必须先明确目标 Vault、备份/回滚方式和安装内容；阶段 9 的仓库内准备不等于正式安装授权。
