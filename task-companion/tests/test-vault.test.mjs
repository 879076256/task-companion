...
test('test vault contains the three Obsidian plugin artifacts', async () => {
	...
	assert.equal(parsedManifest.version, '1.1.1');
...
test('test vault is installed byte-for-byte from the 1.1.1 release', async () => {
	for (const filename of ['main.js', 'manifest.json', 'styles.css']) {
		const [installed, released] = await Promise.all([
			readFile(new URL(filename, installedRoot)),
			readFile(
				new URL(`../release/task-companion-1.1.1/${filename}`, import.meta.url),
			),
		]);
