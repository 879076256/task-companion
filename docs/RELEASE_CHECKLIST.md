...
- [x] 累计投入超过 60 分钟仍显示总分钟数，不出现小时格式。
- [x] 专注和休息自然完成后只准备下一阶段，点击开始前时间不减少，重载后仍保持待开始。
- [x] 25/5/15 轮次、提醒、休息会话隔离及旧数据恢复测试通过。
- [x] package、lockfile、根目录/插件清单、版本映射、安装说明和版本展示统一为 `1.1.1`。
- [x] 完成 build、typecheck、零警告 lint、全部测试和可重复发布构建。
- [x] 提交推送、精确 `1.1.1` 标签、Release 附件上传与远程回读完成。
- [x] 正式 Vault 在检查无活动计时并完成外部备份后覆盖三文件；安装产物、长期数据和主页/CSS 哈希核验通过。覆盖期间用户启动的新一轮计时仅写入 `data.json`，未被中断或覆盖，暂不强制重载。
