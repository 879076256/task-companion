...
- [x] `1.0.1` 提交推送、精确标签、正式 Release 和远程附件回读完成。

## 1.1.0 倒计时提醒与自动番茄循环

- [x] 25 分钟自然完成、5/15 分钟休息循环、每三轮长休息和休息会话隔离测试通过。
- [x] 弹窗、声音和后台系统通知接线测试通过；权限或音频失败不影响计时。
- [x] package、lockfile、根目录/插件清单、版本映射、安装说明和版本展示统一为 `1.1.0`。
- [x] production build、typecheck、零警告 lint、全部测试、可重复发布构建和测试 Vault 安装校验通过。
- [x] 提交推送、精确 `1.1.0` 标签、Release 附件上传与远程回读完成。
- [ ] 正式 Vault 插件与长期数据已外部备份；只覆盖三文件且主页/CSS 哈希不变。
